...
This unvalidated example has limited rule coverage and may miss important
medication risks. It is not reviewed, approved, sponsored, or endorsed by
MNGHA, SFDA, or any other healthcare institution or regulator. Use synthetic
data only. A licensed pharmacist or physician must independently verify all
clinical information using authorized sources.
"""
...
initial_state = {
    "topic": """Educational research prototype for a patient-specific drug-interaction workflow that could be evaluated for future pharmacist use in Saudi healthcare settings. Any possible MNGHA context is aspirational only and has not been reviewed, approved, sponsored, or endorsed by MNGHA. Source rules: prioritize current authorized Saudi product information and publicly available official sources, then relevant published MNGHA/NGHA and KAIMRC evidence, current official clinical guidelines, and recent PubMed systematic reviews, meta-analyses, randomized trials, or large multicenter studies. Exclude commercial blogs and small low-quality studies when stronger evidence exists. Include medication pair, interaction mechanism, evidence-level severity, and patient-specific modifiers including dose, route, age, diagnoses, kidney function (eGFR/CrCl/dialysis), liver function (Child-Pugh/labs), QTc, electrolytes, allergies, pregnancy, and timing. Treat all modifiers except the medication list as optional; if missing, state Not provided, never assume normal, lower confidence, and list what is needed. Never invent internal institutional policy. Require independent pharmacist/physician verification before any clinical action.""",
    "search_query": "",
    "collected_data": [],
    "analyzed_data": [],
    "quality_score": 0,
...
def assess_mngha_patient(profile: Dict[str, Any]) -> str:
    ...
    lines = [
        "EDUCATIONAL MEDICATION-SAFETY PROTOTYPE — NOT FOR CLINICAL USE",
        "Unvalidated, limited rule coverage; synthetic data only.",
        "No MNGHA, SFDA, or institutional review, approval, or endorsement.",
        "",
        "Patient profile",
        f"- Age: {_value(age)}",
    ...
    lines.extend([
        ...
        *[f"- {item}: Not provided" for item in missing],
        "",
        "BOUNDARY",
        "- This educational output does not replace a licensed pharmacist or",
        "  treating prescriber and must not be used to make patient-care decisions.",
        "- The patient must not independently start, stop or change medicines.",
        "- A licensed professional must independently verify all findings using",
        "  authorized institutional policy, interaction references, guidelines,",
        "  and current locally approved product information.",
    ])
    return "\n".join(lines)
